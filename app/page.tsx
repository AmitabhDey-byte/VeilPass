...
export default function Home() {
  ...
  async function sendChat(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      const response = await fetch("/api/chat", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ messages: next, network: selectedNetwork }) });
  ...
  return <div className="app-shell">
    ...
    <AnimatePresence>{showAssistant && <motion.aside className="assistant-drawer" initial={reducedMotion ? undefined : { opacity: 0, x: 28 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: 28 }} transition={{ duration: 0.24 }}><header><div><i>✦</i><span><strong>Veil assistant</strong><small>{chatMode === "gemini" ? "Live via Gemini" : "Calm local mode"}</small></span></div><button className="close-button" type="button" onClick={() => setShowAssistant(false)}>×</button></header><div className="assistant-messages">{chatMessages.map((message, index) => <motion.p className={message.role} key={`${message.role}-${index}`} initial={reducedMotion ? undefined : { opacity: 0, y: 6 }} animate={{ opacity: 1, y: 0 }}>{message.text}</motion.p>)}{chatBusy && <p className="assistant typing">Thinking…</p>}</div><form onSubmit={sendChat}><input value={chatInput} onChange={(event) => setChatInput(event.target.value)} placeholder="Ask about privacy, 1AM, or Midnight…" /><button type="submit" disabled={chatBusy}>→</button></form><small className="assistant-note">Never share a seed phrase, API key, or private credential.</small></motion.aside>}</AnimatePresence>
