...
export default function Home() {
  ...
  async function connectWallet(): Promise<ConnectedAPI | null> {
    if (connected) {
      setWalletApi(null);
    ...
    try {
      ...
      if (status.status !== "connected") throw new Error("The Midnight wallet did not establish a connection.");
  ...
  async function finishProof() {
    if (!deployedProofRef.current) {
      setNotice("Deploy VeilPass with this wallet before submitting a live access proof.");
    ...
    try {
      ...
      setNotice("Private access proof finalized on Midnight.");
    } catch (error) {
      const message = error instanceof Error ? error.message : "Private access proof failed.";
      setWalletError(message);
      setNotice("Private proof was not submitted");
    } finally {
      setProofBusy(false);
  ...
  async function deployContractFromWallet() {
    ...
    try {
      ...
      setNotice("Contract deployed. Full address copied to clipboard.");
    } catch (error) {
      const message = error instanceof Error ? error.message : "Wallet deployment failed.";
      setWalletError(message);
      setNotice("Contract deployment failed");
    } finally {
...
function PrivacyCard({ onOpen, onCopy, copied, contractAddress, onDeploy, deploymentBusy }: { onOpen: () => void; onCopy: () => void; copied: boolean; contractAddress: string; onDeploy: () => void; deploymentBusy: boolean }) {
